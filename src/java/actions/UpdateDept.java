...
public class UpdateDept extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
          ...
          String dept=request.getParameter("department");
           ...
           Connection con=db.Dbconnection.getCon();
            String query="update department set  `graduate_`=?,`name_`=?, `sem_count`=? where id='"+id+"'";
	    ...
	    ps.setString(1, grad.toLowerCase());
            ...
            response.sendRedirect("editing_dept.jsp?updated='yes'&id="+id);
            
        }catch(Exception e){
         System.out.println("Exception :"+e);       
          response.sendRedirect("edit_dept.jsp?failed='yes'");
        }
    }
